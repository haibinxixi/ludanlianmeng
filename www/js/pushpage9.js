...
function UserGetUserWithdrawals() {
    var url = 'http://zl.newditui.com/User/GetUserWithdrawals/' + OPENID + "/" + Token;
...
function UserGetUserAccountingList() {
    var url = 'http://zl.newditui.com/User/GetUserAccountingList/' + OPENID + "/" + Token;
...
function TaskGetTaskShareDetailList() {
    var url = 'http://zl.newditui.com/Task/GetTaskShareDetailList/' + OPENID;
    FuncAjax(url, "", function (obj) {
        if (obj.ResultCode == 0) {
            var data = obj.ResultOBJ;
            if (data.length > 0) {
