...
( function( factory ) {
    ...
    fn._create = function() {
        var that = this,
                ...
                (multiple ? "" : "<span class='ui-icon ui-icon-triangle-1-s"+(bts?'':'')  +"'></span>"),
    ...
    fn.search = function(val) {
        ...
        var _andArr = val.split(' AND '), _orArr;
        var indx = -1, orIndx = -1;
        for (var i = 0, len = data.length; i < len; i++) {
            var rowData = data[i],
                 text = rowData.text.toUpperCase();
            indx = -1;
            for (var andKey in _andArr) {
                if (_andArr.hasOwnProperty(andKey)) {
                    _orArr = _andArr[andKey].split(' OR ');
                    orIndx = -1;
                    for (var orKey in _orArr) {
                        if (_orArr.hasOwnProperty(orKey)) {
                            orIndx = text.indexOf(_orArr[orKey].trim());
                            if (orIndx > -1) {
                                indx = orIndx;
                                break;
                            }
    ...
    fn._createPopup = function() {
        ...
        if (o.search) {
            searchHTML = ["<div class='pq-select-search-div ui-corner-all'>",
                ...
                "<input type='text' class='pq-select-search-input' autocomplete='off' placeholder='expr1 or expr2 and expr3' />",
                "</div>",
                "</div>"].join("");
        }
        ...
        $popup.on({
            change: function(evt) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var rowData = data[i];
                    if (!rowData.hidden && !rowData.disabled && rowData.selected !== checked) {
                ...
                tmpl = function(indx) {
                    if (deselect) {
                        return ["<span class='", clsItem, "' data-id = '", indx, "'>",
                            "<span class='ui-icon ui-icon-close ",closeIcon,"'></span>",
